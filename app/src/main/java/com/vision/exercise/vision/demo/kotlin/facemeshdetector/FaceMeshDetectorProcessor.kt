...
class FaceMeshDetectorProcessor(context: Context) :
  ...
  init {
    ...
    if (com.vision.exercise.vision.demo.preference.PreferenceUtils.getFaceMeshUseCase(context) == FaceMeshDetectorOptions.BOUNDING_BOX_ONLY) {
  ...
  override fun onSuccess(faces: List<FaceMesh>, graphicOverlay: com.vision.exercise.vision.demo.GraphicOverlay) {
    for (face in faces) {
